...
from emotion_data_reader import run
...
def dense_y(y):
    ...
    return csr_matrix((data, (row, y)), shape=(cnt, NUM_LABELS)).toarray()
...
def emotion_cnn(dataset):
    ...
    with tf.name_scope("fc_1") as scope:
        ...
        h_flat = tf.reshape(h_pool2, [-1, int(image_size * image_size * 64)])
...
def main(argv=None):
    train_images, train_labels, valid_images, valid_labels, test_images, test_labels = run(FLAGS.data_dir)
    ...
    with tf.Session() as sess:
        ...
        for step in np.arange(MAX_ITERATIONS):
            ...
            batch_label = dense_y(batch_label)
            ...
            if step % 10 == 0:
                ...
                print("Step: %d Training Loss: %f" % (step, train_loss))

            if step % 100 == 0:
                valid_loss = sess.run(loss_val,
                                      feed_dict={input_dataset: valid_images, input_labels: dense_y(valid_labels)})
                print("%s Step: %d Validation Loss: %f" % (datetime.now(), step, valid_loss))
